...
# Rule 2: No C include patterns
while IFS= read -r line; do
  ERRORS+=("$line")
done < <(grep -n '^\s*#include' "$FILE" | sed 's/^/Import detected — line /')

# Rule 3: No C function definitions (heuristic)
while IFS= read -r line; do
  ERRORS+=("$line")
done < <(grep -nE '^\s*(static )?(void|int|char|float|double|long|unsigned|signed|bool|size_t|FILE)\s+\*?\w+\s*\(' "$FILE" | sed 's/^/Definition detected — line /')
